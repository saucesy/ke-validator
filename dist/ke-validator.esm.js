...
function isURL(url, options) {
  ...
  if (split.length > 1) {
    ...
    var _auth$split = auth.split(':'),
        _auth$split2 = _slicedToArray$1(_auth$split, 2),
        user = _auth$split2[0],
...
function isDecimal(str, options) {
  ...
  if (options.locale in decimal) {
    return !includes$1(blacklist$1, str.replace(/ /g, '')) && decimalRegExp(options).test(str);
  }
...
class Rule {
  ...
  validate(field) {
    ...
    if (!validator$1[this.name](field, this.params)) {
